...
`src/agent/agent-turn.ts` 的 `runAgentTurn()` 当前执行一次受控的工具决策：
...
工具结果目前直接使用业务层生成的确定性文本。M5 会把工具结果回传给模型，实现“执行工具后由模型生成最终回复”的 Agent Loop，并处理多步决策。
